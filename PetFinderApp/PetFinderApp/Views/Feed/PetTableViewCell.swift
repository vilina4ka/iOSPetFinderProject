...
final class PetTableViewCell: UITableViewCell {
    ...
    private let timeAgoLabel: PaddedLabel = {
        let label = PaddedLabel(hPadding: 8)
    ...
    private let foundBadgeLabel: PaddedLabel = {
        let label = PaddedLabel(hPadding: 8)
        label.translatesAutoresizingMaskIntoConstraints = false
        label.text = "Найден"
        label.font = .systemFont(ofSize: 12, weight: .medium)
    ...
    private func setupLayout() {
        ...
        containerView.addSubview(foundBadgeLabel)
        ...
        NSLayoutConstraint.activate([
            ...
            timeAgoLabel.widthAnchor.constraint(greaterThanOrEqualToConstant: 56),
    ...
    func configure(with pet: Pet) {
        ...
        let timeText = pet.timeAgo
        timeAgoLabel.text = timeText
        timeAgoLabel.isHidden = timeText.isEmpty
